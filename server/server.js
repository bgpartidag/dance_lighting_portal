...
mongoose.connect(
	"mongodb://localhost:27017/danceLightDB",
...
app.get("/node_get_show_by_id", function (req, res) {
	...
	Show.findOne({ _id: show_id }, function (err, data) {
...
app.get("/node_get_dance_by_id", function (req, res) {
	...
	Dance.findOne({ _id: dance_id }, function (err, data) {
		if (err || data.length === 0) {
			res.send({
...
app.post("/node_delete_all_cues_by_dance", (req, res) => {
	...
	Cue.deleteMany({ parent_dance: { $in: dance_id } }, {}, (err) => {
		if (err) {
			res.send({
...
app.post("/node_delete_dance_by_id", (req, res) => {
	...
	Dance.deleteOne({ _id: dance_id }, {}, (err) => {
		if (err) {
			res.send({
...
app.post("/node_delete_all_dances_by_show", (req, res) => {
	...
	Dance.deleteMany({ parent_show: { $in: show_id } }, {}, (err) => {
		if (err) {
			res.send({
...
app.post("/node_delete_show_by_id", (req, res) => {
	...
	Show.deleteOne({ _id: show_id }, {}, (err) => {
		if (err) {
			res.send({
